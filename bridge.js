...
(function () {
    // Chaves usadas para state/metrics/snapshots
    const KEYS = {
        STATE: "ps:state:v2",
    ...
    // Utilitarios de datas, status e seletores de projeto
    const utils = {
        todayISO,
    ...
    // Calculo e persistencia de metricas
    const metrics = {
        recalc(state) {
    ...
    // Snapshots de progresso por projeto
    const snapshots = {
        record(state) {
    ...
    // Store principal (estado in-memory + persistencia)
    const store = (() => {
        let state = null;
